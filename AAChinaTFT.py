...
class ModelBase:
    ...
    def __init__(self, 
        ...
        print('max_epochs:', max_epochs, 'batch_size:', batch_size, 'loss_func_metric:', loss_func_metric)
        # fn
        ...
        _checkpoint_callback = ModelCheckpoint(dirpath = _dir + f'cr[{self.scrop}]-yr[{self.val_year}]-en[{self.exp_name}]-bs[{self.batch_size}]', every_n_epochs = 50)

        _dir = '/tf_logs'
        # dir = os.path.join(home_dir, 'data')
        _logger = TensorBoardLogger(_dir, name = f'cr[{self.scrop}]-yr[{self.val_year}]-en[{self.exp_name}]-bs[{self.batch_size}]', \
                                    comment = f'cr[{self.scrop}]-yr[{self.val_year}]-en[{self.exp_name}]-bs[{self.batch_size}]')

        _lr_monitor = LearningRateMonitor(logging_interval = 'epoch')

        _lr_finder  = FineTuneLearningRateFinder(milestones = lr_milestones_list)
        ...
        self.checkpoint = f"cr[{self.scrop}]-yr[{self.val_year}]-en[{self.exp_name}]-bs[{self.batch_size}]"
    ...
    def predict(self,):
        ...
        np.savez(
            f'data/AAAcr[{self.scrop}]-yr[{self.val_year}]-en[{self.exp_name}]-bs[{self.batch_size}]_predict.npz',
    ...
    def inference(self,):
        ...
        np.savez(
            f'data/AAAcr[{self.scrop}]-yr[{self.val_year}]-en[{self.exp_name}]-bs[{self.batch_size}]_inference.npz',
            prediction = experiment['prediction'].numpy(),
            encoder_target = experiment['encoder_target'].numpy(),
            decoder_target = experiment['decoder_target'].numpy(),
            )  
...
class RunTask:
    @staticmethod
    def train_TFT(exp_name, batch_size=24, loss_func_metric='RMSE', max_epochs=600):
