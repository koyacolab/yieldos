...
class _LRFinder:
    ...
    def plot(self, filter_size=100, suggest: bool = False, show: bool = False, ax: Optional["Axes"] = None) -> Optional["plt.Figure"]:
        ...
        losses_filted = signal.filtfilt(coef, 1, losses)

        # Plot loss as a function of the learning rate
        ax.plot(lrs, losses)
        ax.plot(lrs, losses_filted)
        ...
        if suggest:
            ...
            if self._optimal_idx:
                ax.plot(lrs[self._optimal_idx], losses_filted[self._optimal_idx], markersize=10, marker="o", color="red")
    ...
    def suggestion(self, filter_size=100, skip_begin: int = 10, skip_end: int = 1) -> Optional[float]:
        ...
        # from scipy import signal
        coef = np.ones(filter_size) / filter_size
        losses = signal.filtfilt(coef, 1, losses)

        # min_grad = np.gradient(loss[skip_begin:-skip_end]).argmin()
